...
[.spec.domain.watchdog] automaticaly triggers an action via  https://libvirt.org/formatdomain.html#elementsWatchdog[Libvirt]
...
An http://www.kubevirt.io/user-guide/#/workloads/virtual-machines/disks-and-volumes?id=emptydisk[emptyDisk volume]creates an
...
controls whether the associatied VirtualMachine is created.
...
is optional but if not defined will be applied to all `VirtualMachine` objects -
